...
// 🔥 新增：視頻區域內訊息顯示
function showVideoMessage(message, duration = 3000) {
    const overlay = document.getElementById('videoMessageOverlay');
    const text = document.getElementById('videoMessageText');
...
// 🔥 修改原有的 showMessage 函數，手機版時使用視頻內顯示
function showMessage(message, type = 'info') {
    // 手機版且相機已啟動時，使用視頻內訊息
    if (isMobile() && cameraActive) {
        // 清理訊息中的表情符號和多餘文字，只保留核心資訊
        let cleanMessage = message;
        
        if (type === 'success' && message.includes('相機已啟動成功')) {
            if (message.includes('後置鏡頭')) {
                cleanMessage = '✅ 後置鏡頭已啟用';
            } else if (message.includes('前置鏡頭')) {
                cleanMessage = '⚠️ 使用前置鏡頭';
            } else {
                cleanMessage = '✅ 相機已啟動';
            }
        } else if (type === 'info' && message.includes('正在啟動相機')) {
            cleanMessage = '📷 啟動中...';
        } else if (type === 'error') {
            cleanMessage = '❌ 啟動失敗';
        }
        
        showVideoMessage(cleanMessage, type === 'success' ? 2000 : 3000);
        return; // 手機版不再使用頁面頂部訊息
    }
    
    // 桌機版或相機未啟動時，使用原有的頁面頂部訊息
...
function showResultModal(result, capturedBlob = null) {
    ...
    if (result.success && result.book) {
        ...
        if (similarity >= 81) {
            // 高相似度 - 顯示完整資訊
            html = `
                <div class="result-title">✅ 辨識成功！</div>
                <div class="result-book-title">${book.title}</div>
                <div class="result-similarity">
                    相似度: <span class="similarity-high">${similarity}%</span>
                </div>
            `;
            ...
            // 低相似度 - 只顯示警告訊息
            html = `
                <div class="result-title">⚠️ 相似度偏低</div>
...
// ===========================================
// 全域函數 - 確保可以被 HTML onclick 調用
// ===========================================

// 隱藏結果彈出層 - 全域函數
function hideResultModal() {
    console.log('🚪 hideResultModal 被調用');
    ...
    if (cameraActive && videoStream) {
        ...
        console.log('⬆️ 頁面已跳到頂部');
    }
}

// 重新辨識 - 全域函數
function retryIdentification() {
    console.log('🔄 retryIdentification 被調用');
    hideResultModal();
    console.log('✅ 準備重新辨識，請調整書籍位置後再次點擊 AI 辨識');
     
    // 🆕 新增：確保頁面在頂部（如果 hideResultModal 中的滾動有延遲）
    setTimeout(() => {
        window.scrollTo({
            top: 0,
            behavior: 'smooth'
...
document.addEventListener('DOMContentLoaded', function() {
    initScanner();
    console.log('🎉 FastAPI 書籍識別系統載入完成！手機優化版 + 視頻內按鈕 + 透明引導文字 + 視頻內訊息顯示');
});